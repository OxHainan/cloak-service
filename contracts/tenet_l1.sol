...
contract Tenet is Ownable {
    ...
    function withdrawl(address token, address user, uint256 amount) external { // TODO: onlyAppAdmin
        require(tokenRegistered[token], "Token not registered");
        require(balance(token) >= amount, "Token balance lower than the value requested");
        (bool success, bytes memory data) = token.call(
            abi.encodeWithSelector(IERC20Minimal.transfer.selector, user, amount)
        );
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Withdral failed");
    ...
    function registerToken(address tokenAddr) external onlyOwner {
        require(!tokenRegistered[tokenAddr], "Token address already registered");
    ...
    function registerNode(
        address id, string calldata ip, bytes32  mrenclave, uint256 version,
        bytes32 publicKey, bytes calldata report
    ...
    modifier onlyAppAdmin() {
        require(bytes(admin2AppId[msg.sender]).length > 0, "App not registered or permission denied");
        require(apps[admin2AppId[msg.sender]].admin == msg.sender, "Permision denied");
        _;
    }
