...
contract('CloakService', async (accounts) => {
    ...
    it('Test service', async () => {
        const proxy1 = await TransparentProxy.deployed();
        const service = await Service.deployed();
        const proxy = await ITransparentUpgradeableProxy.at(proxy1.address);
        await proxy.upgradeTo(await service.stateFactory());
    ...
    it('should contract enchaned', async () => {
        ...
        // await service.cancel(TransparentProxy.address)
    })
